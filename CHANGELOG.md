...
- **Fix: an unresolvable system prompt fails the turn instead of the process** — the provider still refuses a prompt no `before_agent_start` recorded, because forwarding it would hand Claude Code Pi's own harness and the 400 that comes with it. But it refused by throwing, and pi only converts a thrown provider error into a failed assistant message for callers that own an `AgentSession`. A low-level `agentLoop` caller — a background worker such as an observational-memory observer — calls the provider from a detached promise, where the throw became an `uncaughtException` that ended pi. The refusal is now a terminal error event on the returned stream, so such a caller sees a failed turn and the session survives.
